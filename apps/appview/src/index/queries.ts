...
export function parseRecord<T = Record<string, unknown>>(row: RecordRow & { counts?: Record<string, number> }): IndexedRecord<T> {
...
export async function listCollection<T = Record<string, unknown>>(
  ...
  return { records: res.records.map((r) => parseRecord<T>(r)), cursor: res.cursor }
}
...
export async function getRecordByUri<T = Record<string, unknown>>(
  ...
  const res = await indexer.contrail.query(
    short,
    { did: parts.did, limit: 100 },
    indexer.db,
  )
  const row = res.records.find((r) => r.uri === uri)
  return row ? parseRecord<T>(row) : null
}
...
export async function eventsInWindow(
  ...
  const res = await indexer.contrail.query(
    'event',
    { rangeFilters: { startsAt: { min: from, max: to } }, limit, sort: { recordField: 'startsAt', direction: 'asc' } },
    indexer.db,
  )
...
function safeShort(short: string): string {
  if (!/^[A-Za-z0-9_]+$/.test(short)) throw new Error(`unsafe collection short name: ${short}`)
  return short
}
