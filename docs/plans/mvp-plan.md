...
- Create: `apps/web/src/routes/OAuthConfirmScreen.tsx` (the verbatim hard-confirm copy from Global Constraints, button "Continue anyway" → `window.location = api.auth.oauthStartUrl(true)`, button "Go back and create a new identity" → `/signin`; if the start endpoint returns 503 `OAuthUnavailable`, show "Signing in with an existing account isn't available on this server yet." — do not show a broken redirect)
...
- Test: `apps/appview/test/take-ownership.test.ts` (flips `isCustodial`, removes the stored credential, reveal token is single-use, second reveal is 410; against the live PDS when reachable — the smoke host can afterwards `createSession` with the revealed password)
