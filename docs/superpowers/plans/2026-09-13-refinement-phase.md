...
### Task 19: Interoperability and publication-tier audit (added 2026-09-13 at Benjamin's request)

**Files:**
- Create: `docs/interop-audit.md`
- Modify (only where the audit finds a defensible gap): `apps/appview/src/http/routes/*.ts`, `apps/appview/src/lib/events.ts`, `apps/appview/src/contrail.config.ts`, `packages/lexicons/lexicons/freeschool/draft/*.json` (descriptions only, never new fields on borrowed records), `docs/architecture.md`

**Deliverable 1 — the tier matrix.** For every piece of data the app holds (events, occurrences, series, listings, RSVPs, attendance, requests and interest, skill nodes, skill claims by visibility, attestations, badges/roles, member profile fields, handles, resources/notes, policy, school record, moderation, invites, newsletter, feedback), one row stating: where it lives today (record in whose repo / app-side table), which of the four tiers it reaches — **firehose** (any ATProto app via relay/Jetstream), **peer skools** (other Free School AppViews that list us as a peer), **within this skool** (signed-in members), **not published** — the reason (cite R9, PRD principle, or Lucian/Lex decision), and whether the reason still holds. Every "not published" or "within skool only" row must carry a well-thought-through reason or be flagged as a gap.

**Deliverable 2 — interoperability inventory.** (a) Outbound: which records are valid against their canonical lexicons (`community.lexicon.calendar.*`, `coop.lexicon.*`) so Smoke-Signal-style calendar apps, the Arbiter, and COhere can consume them; tag routing (`skillshare`, `free-school`); what a generic ATProto client sees on a member's repo (skill claims, events) and on the school's repo. (b) Inbound: what `PEER_PDS_HOSTS` / `PdsChangeSource` actually ingest from peers today, what a second skool instance would need to federate (peer registry, school records, listings, skill authority sharing), and what breaks. (c) Identity: handles under `freeskool.xyz`, DID documents, OAuth door, "take ownership" path — can a member move their repo to another PDS and keep their Free School history? (d) Skill profiles and attestations across skools: claims travel with the member's repo; attestations are app-side — document the double-opt-in public attestation record design (both parties write a record; the AppView pairs them) as the path to cross-skool badges, with the R9 analysis. (e) Badges/roles: derived and never minted; state what a peer skool may derive from our public records.

**Deliverable 3 — fixes.** For each gap where publication is safe and valuable (e.g. a public endpoint that omits a field already public in the record; a record written without a `$type`/tag a consumer needs; a listing not routed), make the change with a test. For each gap that must stay closed, the matrix row is the record of why. Nothing in this task may add a field to a borrowed lexicon or publish a DID its holder did not write.

- [ ] Step 1: Inventory by reading `contrail.config.ts`, `lib/events.ts` (`routeListing`, tags), `routes/calendar.ts`, `routes/school.ts`, `routes/skills.ts`, `routes/knowledge.ts`, `lib/membership-claims.ts`, `sync/*`, `packages/lexicons/lexicons/**`, `docs/architecture.md`, `docs/research-brief-v0.2.md` §placement, `docs/lucian-review-packet.md`.
- [ ] Step 2: Write `docs/interop-audit.md` with the matrix, the inventory, and a numbered gap list (severity, recommendation, who decides).
- [ ] Step 3: Implement the safe gaps; tests; commit `docs+feat(interop): publication-tier audit and fixes`.
